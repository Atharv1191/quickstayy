        ...
        <div className='grid grid-col-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
            {
                exclusiveOffers.map((item)=>(
                    <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 px-4 md:pt-18 rounded-xl text-white bg-no-repeat bg-cover bg-center'style={{backgroundImage: `url(${item.image})`}}>
                        ...
                        <button className='flex items-center gap-2 font-medium cursor-pointer mt-4 mb-5'>
                            View Offers
                            <img src={assets.arrowIcon} alt="arrorw" className='invert group-hover:translate-x-1 transition-all' />
                        </button>
