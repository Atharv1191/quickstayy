...
import { FaWifi, FaSwimmingPool, FaParking, FaConciergeBell, FaMapMarkerAlt } from 'react-icons/fa'
import StarRating from '../components/StarRating'
import { useAppContext } from '../context/AppContext'

// Map your amenities to React Icons components here
const facilityIcons = {
  Wifi: FaWifi,
  Pool: FaSwimmingPool,
  Parking: FaParking,
        ...
        {filteredRooms.map((room) => (
          <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
            ...
            <div className='md:w-1/2 flex flex-col gap-2'>
              ...
              <p
                onClick={() => { navigate(`/rooms/${room._id}`); scrollTo(0, 0) }}
                className='text-gray-800 text-3xl font-polyfair cursor-pointer'
              >
                {room.hotel.name}
              </p>
              ...
              <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                {room.amenities.map((item, index) => {
                  const IconComponent = facilityIcons[item]
                  return (
                    <div key={index} className='flex items-center gap-2'>
                      {IconComponent ? (
                        <IconComponent className='w-5 h-5' title={item} />
                      ) : (
