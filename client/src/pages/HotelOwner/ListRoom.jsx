...
const fetchRooms = async () => {
  try {
    const token = await getToken(); // move this outside to keep things clean
    ...
    if (data.success) {
      setRooms(data.rooms); // expected to be an array
    } else {
                                ...
                                <tr key={index}>
                                    ...
                                    <td className='py-3 px-4 text-red-500 border-t border-gray-300 text-center'>
                                        <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                                            <input onChange={()=>toggleAvailibility(item._id)} 
                                                type="checkbox" 
